...
$rs_control = mysql_fetch_array($rs_control_result);
 if($rs_control){
  $error_alert .= $lang_['publicity']['INSERT_UPDATE_DUPLICATE_ITEM_ERROR'].'<br/>';
 }
/***************** /control on duplicate codes ***********/ 
...
if(!isset($_FILES)) $_FILES = $HTTP_POST_FILES;
 ...
 if($ext_error != '') $error_alert .= str_replace('{allowed_files}',implode(",", $allowed_types),$lang_['publicity']['UPLOAD_EXTENSION_FILES_ERROR']).'<br/>';
 if($generic_error != '') $error_alert .= $lang_['publicity']['UPLOAD_GENERIC_ERROR'];
/***************** / upload ***********/	
...
/* get old product data */
  $res_p = execute('select file_name,name,point,categories,id from '.$table_name.' where id  = '.$_POST['id']);
