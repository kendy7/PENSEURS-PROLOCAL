...
if (isset($_POST['userid_r']) && $_POST['userid_r']){
 $sqlsearch .= " userid = '".str_db($_POST['userid_r'])."' and ";
}
if ((isset($_POST['enabled_r']) && $_POST['enabled_r']) || !isset($_POST['disabled_r'])){
 $sqlsearch_or .= " payed_bonu = 0 or ";
}
...
if ($sqlsearch_or != ''){
 ...
 if ($sqlsearch != ''){ 
  ...
  $sql = ' select * from ag_bonus'. $sqlsearch.' and montant <>0.00';
...
while ($rs = mysql_fetch_array($rs_result)) {
   ...
   $img_enabled = '<span class="btn action-enable'.($rs['payed_bonu'] ? ' processed btn-success' : ' btn-warning').'" id="'.$rs['id'].'" style="cursor:pointer;margin-right:10px;" rel-tooltip="tooltip" title="'.($rs['payed_bonu'] ? $lang_['clients_accounts']['TOOLTIP_DISABLE_LOGIN'] : $lang_['clients_accounts']['TOOLTIP_ENABLE_LOGIN']).'"><i class="icon icon-white icon-'.($rs['payed_bonu'] ? 'unlocked' : 'locked').'"></i></span>';
   
   $status = $rs['payed_bonu'] ? 'icon-check' : 'icon-close';
   $slect_checkbox = '<input type="checkbox" id="'.$rs['id'].'" value="'.$rs['id'].'" name="box_delete[]" class="check_selezione" align="absmiddle" />';		
	/* here start the part that provides to return data ----- from here to... (NOT CHANGE/DELETE ID ADN SELECT_CHECKBOX KEY, NOT DELETE ACTIONS KEY-YOU CAN EDIT IT ONLY)*/  
	  $initial_array[] = array(
	   ...
	   "payed_bonu" => str_json('<span style="display:none">'.$rs['payed_bonu'].'</span><i class="icon icon-color '.$status.'"></i>'),
	...
	if(!empty($initial_array)){
		...
		*/
		   ...
		   $result .= '"payed_bonu":"'.$item['payed_bonu'].'",';
		   $result .= '"archive":"'.$item['archive'].'",';
		   $result .= '"Datee":"'.$item['phone'].'",';
