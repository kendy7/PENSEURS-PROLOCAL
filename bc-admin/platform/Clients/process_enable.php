 ...
 if(!$rs['enabled']){
     ...
     // $message = str_replace('{shop_url}','<a href"'.$shop_url.'" target="_blank">'.$shop_url.'</a>',$message); 
	 	 require_once(theme_rel_path.'/emails/form_client.php');
				 ...
				 $message = str_replace('{template_logo}',$logo,$email_template_client_registration);
				 $message = str_replace('{CodeMembre}',$rs['CodeMembre'],$message);
				 $message = str_replace('{name}',$rs['name'],$message);
				 $message = str_replace('{lastname}',$rs['lastname'],$message);
				 $message = str_replace('{userid}',$rs['userid'],$message);
	 $mail = new PHPMailer();
	  if($smtp_email){
	   $mail->IsSMTP();
	   $mail->Port = $smtp_port;
	 ...
	 $mail->CharSet = 'UTF-8';
	 $mail->From = "ASCENSION GROUP"; 
	 $mail->FromName = "ASCENSION GROUP"; 
