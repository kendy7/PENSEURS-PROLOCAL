...
if (isset($_POST['id_r']) && $_POST['id_r']){
 $sqlsearch .= " id = '".str_db($_POST['id_r'])."' and ";
}
if (isset($_POST['idcl_r']) && $_POST['idcl_r']){
 $sqlsearch .= " id_client = '".str_db($_POST['idcl_r'])."' and ";
}
if ((isset($_POST['enabled_r']) && $_POST['enabled_r']) || !isset($_POST['disabled_r'])){
 $sqlsearch_or .= " payed_com = 0 or ";
}
...
if ($sqlsearch_or != ''){
 ...
 if ($sqlsearch != ''){ 
  ...
  $sql = ' select * from ag_commission'. $sqlsearch.' and montant <>0.00';
...
while ($rs = mysql_fetch_array($rs_result)) {
   ...
   $img_enabled = '<span class="btn action-enable'.($rs['payed_bonu'] ? ' processed btn-success' : ' btn-warning').'" id="'.$rs['id'].'" style="cursor:pointer;margin-right:10px;" rel-tooltip="tooltip" title="'.($rs['payed_com'] ? $lang_['clients_accounts']['TOOLTIP_DISABLE_LOGIN'] : $lang_['clients_accounts']['TOOLTIP_ENABLE_LOGIN']).'"><i class="icon icon-white icon-'.($rs['payed_com'] ? 'unlocked' : 'locked').'"></i></span>';
   
   $status = $rs['payed_com'] ? 'icon-check' : 'icon-close';
   $slect_checkbox = '<input type="checkbox" id="'.$rs['id'].'" value="'.$rs['id'].'" name="box_delete[]" class="check_selezione" align="absmiddle" />';		
	/* here start the part that provides to return data ----- from here to... (NOT CHANGE/DELETE ID ADN SELECT_CHECKBOX KEY, NOT DELETE ACTIONS KEY-YOU CAN EDIT IT ONLY)*/  
	  $initial_array[] = array(
	   "id" => $rs['id'],
	   "id_client" => str_json($rs['id_client']),
	   "montant" => str_json($rs['montant']),
	   "archive" => str_json($rs['archive']),
	   "Datee" => str_json($rs['Datee']),
	   "payed_com" => str_json('<span style="display:none">'.$rs['payed_com'].'</span><i class="icon icon-color '.$status.'"></i>'),
	   "slect_checkbox" => str_json($slect_checkbox),
	   "actions" => str_json($img_enabled.$img_edit.$img_info)
	  );   
}
	$result  = '{';
	$result .= '"page":'.$pag.',';
	...
	if(!empty($initial_array)){
		...
		*/
		   ...
		   $result .= '"id_client":"'.$item['id_client'].'",';
		   $result .= '"montant":"'.$item['montant'].'",';
		   $result .= '"payed_com":"'.$item['payed_com'].'",';
		   $result .= '"archive":"'.$item['archive'].'",';
		   $result .= '"Datee":"'.$item['Datee'].'",';
