...
$('.retrieve-condition').on('click',function(e){
					 e.preventDefault();
					 $('#retrieve-data-modal-condition').modal('show');
});
		 ...
		 /*///////// STYLE FOR RESPONSIE MENU //////////////////*/
		  ...
		  /*////// MY ACCOUNT ACTIONS ////////*/
			 ...
			 $('#change-password','.myaccount-page').on('click',function(){
				...
				// script d'annulation de commande'
				$('.myaccount-page #step-all-orders .info').on('click',function(){
					 $id = $(this).attr('data-id');
					  $.ajax({
						type:'POST',
						beforeSend:function(){
						...
						data: {id:$id},
						url:$('body').data('abs_client_path')+'/cancel_orders.php',
						success:function(data){	
				...
				$('.myaccount-page #step-ar-order .info-orde').on('click',function(){
					 $id = $(this).attr('data-id');
					  $.ajax({
						type:'POST',
						beforeSend:function(){
				...
				$('.myaccount-page #step-ar-order .delete_orders').on('click',function(){
					 $id = $(this).attr('data-id');
					  $.ajax({
						type:'POST',
						beforeSend:function(){
						...
						data: {id:$id},
						url:$('body').data('abs_client_path')+'/delete_arc_orders.php',
						success:function(data){	
							$.loader.hide();
							url:$('body').data('abs_client_path')+'/Rapport.php?type=MENU_ALL_ORDERS'
							$.bootalert({
								ID        : 'alert-info-order-modal',
								LabelText : $(data).filter('.return').attr('data-label'),
								TypeLabel : $(data).filter('.return').attr('data-label-type'),
						  ...
						  break;
						   case 'need_payed':	
							$.bootalert({
								...
								LabelText : __('general_warning_title'),
								TypeLabel : 'warning',
								BodyText  : __('account_not_payed_message'),
								TypeBody  : 'warning'
							});							  
								$('input:text,input:password','#top-login-form').val('');					  
						  break;
				  ...
				  success:function(data){					  
					  if(data != 'logged'){
					    ...
					    switch(data){
						  ...
						  break;
						  case 'need_payed':	
						    var $body_text = '<strong>'+__('general_warning_title')+'</strong><br/>'+__('account_not_payed_message'),
								$body_type = 'alert-warning';
								$('input:text,input:password','#login-form').val('');					  
						  break;
						  default:
...
/*//FIN//*/
			...
			/*//// CHECKOUT ////*/
			function get_address($name,$lastname,$address,$city,$reference,$email,$phone,$birthdate){
				...
				var address = __('client_address').replace('{name}',ucwords($name+' '+$lastname))
						 ...
						 .replace('{phone}',$phone)
						 .replace('{email}',$email)
						 .replace('{birthdate}',($birthdate != '' ? '<abbr title="Birthdate">Date Naiss: </abbr> '+$birthdate+'<br/>' : ''));	
				return '<address>'+address+'</address>';
			}
			$('[data-steps-rel^="fstep_"]:last').on('classadded', function(ev, newClasses) {
			  if(newClasses == 'current-step'){
				  var $billing_address = get_address($('#name').val(),$('#lastname').val(),$('#address').val(),$('#city').val(),$('#reference').val(),$('#email').val(),$('#phone').val(),$('#birthdate').val()),
				      $payment_address = get_address($('#names').val(),$('#lastnames').val(),$('#addresss').val(),$('#citys').val(),$('#zipcodes').val(),$('#emails').val(),$('#phones').val(),$('#birthdates').val());
