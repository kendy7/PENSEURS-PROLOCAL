 ...
 mysql_query('LOCK TABLE '.$table_prefix.'cart WRITE,'.$table_prefix.'products WRITE,'.$table_prefix.'orders WRITE,'.$table_prefix.'categories WRITE,'.$table_prefix.'clients WRITE');
		...
		while($rs_cart = mysql_fetch_array($cart_items))
		{
		    $total_qta = 0;
			  ...
			  $point=$point+$rs_cart['point'];
			  
              $unitPoint=$rs_cart['point'];
		...
		/** save order into ORDERS TABLE **/
		 $billing_address = array(
		                      "name" => $_POST['is_company'] != 'private' ? ucwords($_POST['name']) : ucwords($_POST['name'].' '.$_POST['lastname']),
							  "address" => $_POST['address'],
							  "city" => $_POST['city'],
							  "reference" => $_POST['reference'],
							  "phone" => $_POST['phone'],
							  "birthdate" => $_POST['fbirthdate'] != '' ? $_POST['birthdate'] : '',
							  "email" => $_POST['email']
		                    );
		 $shipping_address = array(
		                      "name" => $_POST['is_company'] != 'private' ? ucwords($_POST['names']) : ucwords($_POST['names'].' '.$_POST['lastnames']),
							  "address" => $_POST['addresss'],
							  "city" => $_POST['citys'],
							  "reference" => $_POST['zipcodes'],
							  "phone" => $_POST['phones'],
							  "birthdate" => $_POST['faxs'] != '' ? $_POST['faxs'] : '',
							  "email" => $_POST['emails']
		                    );
	...
	if(plugin_exsists('multitaxes') && !empty($arr_multitaxes)){
	 ...
	 foreach($taxes_sum as $key => $val){
        ...
        /** insert order into database **/
		 ...
		 $record .= 'products_list,data,subtotal,grandtotal,tax,commissions,point,shipping_price,payment_method,payment_price,billing_address,shipping_address,code_order';
		 if(plugin_exsists('multitaxes') && !empty($arr_multitax_order)){
