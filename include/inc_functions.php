					...
					// {
					   ...
					   // do
					    // {
							  ...
							  // $req= execute("Select reference from clients where reference=".$Code."");
							  // //execute($req);
...
function get_initial_user_session(){
  ...
  if(!isset($_COOKIE['initial_user_session'])){
	@setcookie('initial_user_session', $radom_session, time()+((3600*1)*$cookies_persistence),'/');
  }
